...
def create_file(path: str, content: str) -> dict:
    """Create a new file in the sandbox; creates parent directories if needed. Example: create_file("hello.txt", "hi")."""
    p = _safe(path)
    if p.exists():
...
def create_calendar_event(
    title: str, date: str, reminders: list[str] | None = None
) -> dict:
    """Create a simple iCalendar (.ics) all-day event in the sandbox calendar folder.
    `date` may be 'DD Month YYYY' (e.g. '15 May 2026') or 'YYYY-MM-DD'.
    `reminders` is a list of ISO 8601 durations like 'P14D' (2 weeks) or 'PT0S' (on day).
    Returns the sandbox path and a short preview.
    """
    caldir = SANDBOX / "calendar"
    caldir.mkdir(parents=True, exist_ok=True)

    # Parse date strings in a couple common formats
    dt = None
    for fmt in ("%d %B %Y", "%Y-%m-%d", "%d %b %Y"):
        try:
            dt = datetime.strptime(date, fmt).date()
            break
        except Exception:
            continue
    if dt is None:
    ...
    uid = f"cal-{uuid.uuid4().hex[:12]}@local"
    ...
    if reminders:
        for rem in reminders:
            # VALARM TRIGGER is negative offset before event
            lines.extend(
                [
                    "BEGIN:VALARM",
                    "ACTION:DISPLAY",
